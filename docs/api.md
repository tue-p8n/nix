...
#### `p8n.uv.readProject (workspaceRoot | { workspaceRoot, ... })` (Alias: `p8n.uv.loadProject`)
...
- **`.shell { name, accelerator?, ... }`**: Shortcut for `(project.build args).shell`.
- **`.oci { name, accelerator?, ... }`**: Shortcut for `(project.build args).oci`.
- **`.sif { name, accelerator?, ... }`**: Shortcut for `(project.build args).sif`.
...
#### `p8n.latex.readProject (src | { src, ... })` (Alias: `p8n.latex.loadProject`)
Inspects a LaTeX repository root (auto-detecting `main.tex` or `paper.tex` and `.latexmkrc`) and returns:
- **`.document { name, main?, ... }`** (or **`.build`**): Builds the document PDF.
- **`.shell { ... }`**: Interactive TeX authoring shell.
...
#### `p8n.typst.readProject (src | { src, ... })` (Alias: `p8n.typst.loadProject`)
Inspects a Typst document root (auto-detecting `main.typ`) and returns `.document` (or `.build`) and `.shell`.
